...
def setup_working_directories(config_vars):
    ...
    config_vars["raw_images_dir"] = os.path.join(config_vars["root_directory"], '0_raw_images/')
    config_vars["raw_annotations_dir"] = os.path.join(config_vars["root_directory"], '1_raw_annotations/')
    ...
    config_vars["normalized_images_dir"] = os.path.join(config_vars["root_directory"], '2_norm_images_merged/')
    config_vars["boundary_labels_dir"] = os.path.join(config_vars["root_directory"], '3_boundary_labels/')
...
def read_data_partitions(config_vars, load_augmented=True):
    ...
    if load_augmented:
        files = glob.glob(config_vars["root_directory"] + "2_norm_images/*_aug_*.png")
...
def setup_experiment(config_vars, tag):
    ...
    # Output dirs
    config_vars["experiment_dir"] = os.path.join(config_vars["root_directory"], tag + "/out/")
    ...
    config_vars["best_model"] = config_vars["root_directory"] + "experiments/" + tag + "/_"
